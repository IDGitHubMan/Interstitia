...
f.write('\t\t\telse:\n')
f.write('\t\t\t\tcollect[row[0]] = {"maker":row[3],"description":row[1],"tags":row[7],"type":row[2],"contributors":row[4],"thumb":row[8],"formatted":row[9]}\n')
...
f.write('\t\t\t\telse:\n')
f.write('\t\t\t\t\thtml=open(fileString,"wb")\n')
f.write('''\t\t\t\t\thtml.write('{% extends "base.html" %}'.encode() )\n''')
f.write('\t\t\t\t\thtml.close()\n')
